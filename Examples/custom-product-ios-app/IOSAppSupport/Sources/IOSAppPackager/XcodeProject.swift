...
enum XcodeProjectGenerator {
    static func generate(
        ...
        let copiedBundles = configuration.resources.embedAllSwiftPMResources
            ? try stageResourceFiles(resourceFiles, beneath: copiedBundlesRoot)
            : []
        ...
        let projectSpec = try makeProjectSpec(
            ...
            platform: platform,
            configuration: configuration,
            assetCatalogPaths: catalogs.map { relativePath(of: $0, under: root) },
            resourceBundlePaths: copiedBundles.map { relativePath(of: $0, under: root) },
    ...
    private static func stageResourceFiles(
        _ resourceFiles: [URL],
        beneath destinationRoot: URL
    ) throws -> [URL] {
        let fileManager = FileManager.default
        var bundles: [String: URL] = [:]

        for source in resourceFiles.sorted(by: { $0.path < $1.path }) {
            let components = source.pathComponents
            guard let bundleIndex = components.lastIndex(where: { $0.hasSuffix(".bundle") }) else {
                throw PackagerError(
                    "SwiftPM resource file is not contained in a resource bundle: \(source.path)"
                )
            }
            let bundleName = components[bundleIndex]
            let relativeComponents = components.dropFirst(bundleIndex + 1)
            guard !relativeComponents.isEmpty else {
                throw PackagerError("resource input must name a file, received bundle root \(source.path)")
            }

            let bundle = destinationRoot.appendingPathComponent(bundleName, isDirectory: true)
        ...
        return bundles.values.sorted(by: { $0.path < $1.path })
    }
    ...
    private static func makeProjectSpec(
        ...
        let sources: [[String: Any]] = [["path": "Generated/LinkAnchor.swift"]]
            + assetCatalogPaths.map { ["path": $0, "buildPhase": "resources"] }
        ...
        if !resourceBundlePaths.isEmpty {
            let commands = resourceBundlePaths.map { path in
                let name = URL(fileURLWithPath: path).lastPathComponent
                return "/usr/bin/ditto \"${PROJECT_DIR}/\(path)\" "
                    + "\"${BUILT_PRODUCTS_DIR}/\(name)\""
            }
            target["preBuildScripts"] = [[
                "name": "Stage SwiftPM resource bundles",
                "script": (["set -euo pipefail"] + commands).joined(separator: "\n"),
                "inputFiles": resourceBundlePaths.map { "$(PROJECT_DIR)/\($0)" },
                "outputFiles": resourceBundlePaths.map {
                    "$(BUILT_PRODUCTS_DIR)/" + URL(fileURLWithPath: $0).lastPathComponent
                },
            ]]
        }
        return [
