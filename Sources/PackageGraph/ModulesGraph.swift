...
enum PackageGraphError: Swift.Error {
    ...
    /// An artifact product cannot be consumed as an ordinary target product dependency.
...
extension PackageGraphError: CustomStringConvertible {
    public var description: String {
        ...
        case .unsupportedCustomProductDependency(let moduleName, let productName, let productPackage):
            return "target '\(moduleName)' cannot depend on artifact product '\(productName)' from package '\(productPackage)'; artifact products are built by product-builder plugins and cannot be linked as target dependencies"
        }
    }
}
