...
/// commands that create an artifact product.
@available(_PackageDescription, introduced: 6.3)
public struct ProductBuilderInput {
    /// The artifact product being built. Its `targets` are the targets selected in
    /// the package manifest.
    public let product: Product

    /// A stable identifier for the artifact product kind, supplied by its definition library.
...
/// A declarative plan for creating the final outputs of an artifact product.
///
...
public enum ProductBuilderPlanValidationError: Error, CustomStringConvertible {
    ...
    case inputAlsoDeclaredAsOutput(URL)
    ...
    public var description: String {
        ...
        case .inputAlsoDeclaredAsOutput(let url):
            return "product builder command declares '\(url.path)' as both an input and an output"
...
extension ProductBuilderPlan {
    ...
    public func validate(outputDirectory: URL) throws {
        ...
        for command in self.commands {
            ...
            let outputs: [URL]
            switch command {
            case .buildCommand(_, _, _, _, let inputFiles, let outputFiles):
            ...
            let normalizedInputs = Set(inputs.map(\.standardized))
            for output in outputs {
                let output = output.standardized
                guard !normalizedInputs.contains(output) else {
                    throw ProductBuilderPlanValidationError.inputAlsoDeclaredAsOutput(output)
                }
